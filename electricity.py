...
def insert_hvac_data(refrigerant, mass_leak, emission):
    conn = sqlite3.connect("emissions.db")
    c = conn.cursor()
    c.execute("INSERT INTO HVACEmissions (Refrigerant, MassLeak, Emission) VALUES (?, ?, ?)", 
...
def show_electricity_hvac_calculator():
    ...
    if st.button("Calculate Electricity Emission"):
    ...
    if st.button("Calculate HVAC Emission"):
